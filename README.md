...
Loja online e serviço de prova ao domicílio de **A Dona Lingerie**.

**Ver o design agora:** abra `preview/index.html` — é o site completo numa página autónoma (hero com tecido animado, catálogo, ficha de materiais e formulário de marcação em 3 passos), sem precisar de instalar nada.
...
Ou arraste os ficheiros para o site do GitHub — já não há nomes de pasta com
...
├── app/                  App Router
│   ├── page.tsx          Homepage
│   ├── agendar/          Marcação (formulário + WhatsApp)
...
│   ├── whatsapp.ts       Links de partilha e marcação
...
**O 3D nunca atrasa a primeira pintura.** `CenaHero` entra por `next/dynamic` com `ssr: false` e só monta após o primeiro frame. Quem tem `prefers-reduced-motion` ativo recebe um gradiente de seda estático — bonito e a custo zero. No telemóvel a malha cai de 144² para 64² segmentos e o DPR fica limitado a 1.5.
...
**Agendamento à prova de corrida.** Há um índice único em `(consultora_id, inicio_em)`: se duas clientes pedirem o mesmo horário no mesmo instante, o Postgres devolve `23505` e a segunda recebe *"este horário acabou de ser reservado"* em vez de um duplo agendamento. Marcações exigem 24 h de antecedência e há rate-limit de 5/hora por IP.
...
Além do habitual (p95, taxa de erro, memória), o dashboard mostra `adona_agendamentos_criados_total` por formato e dispara o alerta `SemAgendamentos` se passarem 6 horas sem nenhuma marcação — normalmente o primeiro sinal de que o formulário partiu silenciosamente.

## WhatsApp

Partilha e marcação por `wa.me` — o deep link oficial, sem SDK e sem peso no
bundle. Botão flutuante em todo o site, partilha em cada peça, e um bloco de
marcação rápida (nome + cidade) na página `/agendar`.
...
- Testes E2E com Playwright nos três passos da marcação
